...
public class ForeignFactoryAction extends CommonService {
     ...
     * @throws
     */
    public SearchMap foreignFactoryList(SearchMap searchMap) {

    	if ("".equals(StaticUtil.nullToBlank((String)searchMap.get("findDeptId")))) {
        	
			searchMap.put("findDeptId", getStr("bsc.popup.foreignFactory.getScDeptMapping", searchMap));
		}

        searchMap.addList("deptInfo", getList("bsc.popup.foreignFactory.getDeptInfo", searchMap));

    	searchMap.addList("itemInfo", getList("bsc.popup.foreignFactory.getItem", searchMap));

        return searchMap;
    }

    /**
     * 해외 공장등록 심사 매출액 데이터 조회(xml)
     * @param
     * @return String
     * @throws
     */
    public SearchMap foreignFactoryList_xml(SearchMap searchMap) {

    	String itemCd = searchMap.getString("findItemCd");

    		searchMap.addList("list", getList("bsc.popup.foreignFactory.getList", searchMap));
    ...
    public SearchMap excel(SearchMap searchMap) {
    	String excelFileName = "해외 공장등록 심사 매출액 실적";
    	String excelTitle = "해외 공장등록 심사 매출액 실적";
    	...
    	searchMap.put("excelDataList", (ArrayList)getList("bsc.popup.foreignFactory.getList", searchMap));
    	return searchMap;
    }
