...
public class EntSupportActIsoAction extends CommonService {
    ...
    public SearchMap entSupportActIsoList(SearchMap searchMap) {
    	...
    	if("".equals(StaticUtil.nullToBlank(deptId))) {
    		searchMap.addList("dept", getList("bsc.popup.entSupportActIso.getDept", searchMap));
    		searchMap.put("findDeptId", searchMap.getDefaultValue("dept", "DEPT_ID", 0));
    	}
    	
    	//최상위 평가조직 조회
        searchMap.addList("deptInfo", getList("bsc.popup.entSupportActIso.getDeptInfo", searchMap));
