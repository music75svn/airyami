...
FSMenu.prototype.show = function(mN) { with (this)
{
 ...
 if (!nested) mtsTimer = setTimeout(myName + '.menus.root.over()', 10);
 	...
 	try{
	 	...
	 	var subMenuUl = document.getElementById(mN);
...
/*
addEvent(window, 'load', new Function('with (' + myName + ') { var m = menus["'+menuID+'"], pM = menus["' + parUL.id + '"];' +  (sOC ? 'if ((pM && pM.child) || (m && m.visible))' : '') + ' show("' + menuID + '", this) ; '));
*/
/*
window.onload = function('with (' + myName + ') { ' +
   'var m = menus["'+menuID+'"], pM = menus["' + parUL.id + '"];' +
   (sOC ? 'if ((pM && pM.child) || (m && m.visible))' : '') +
   ' show("' + menuID + '", this) }');
*/
