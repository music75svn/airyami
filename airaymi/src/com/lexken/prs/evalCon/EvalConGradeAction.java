/*************************************************************************
* CLASS 명      : EvalConGradeAction
* 작 업 자      : 김효은
* 작 업 일      : 2014년 3월 19일 
* 기    능      : 직원개인기여도평가 평가등급배분표 관리
* ---------------------------- 변 경 이 력 --------------------------------
...
public class EvalConGradeAction extends CommonService {
    ...
    /**
     * 직원개인기여도평가 평가등급배분표 관리 조회
     * @param      
     * @return String  
     * @throws 
     */
    public SearchMap evalConGradeList(SearchMap searchMap) {

    	String year = searchMap.getString("findYear");
    	searchMap.put("year", year);
    	
    	searchMap.addList("itemList", getList("prs.evalCon.evalConGrade.getCodeList", searchMap));
    	
        return searchMap;
    }
    
    /**
     * 직원개인기여도평가 평가등급배분표 관리 데이터 조회(xml)
     * @param      
     * @return String  
     * @throws 
     */
    public SearchMap evalConGradeList_xml(SearchMap searchMap) {
        
    	/**********************************
         * 등급배분표 등급 가져오기
         **********************************/
    	ArrayList itemList = (ArrayList)getList("prs.evalCon.evalConGrade.getCodeList", searchMap);
        ...
        searchMap.addList("list", getList("prs.evalCon.evalConGrade.getEvalConGradeList", searchMap));
        
        //searchMap.addList("list", getList("prs.evalCon.evalConGrade.getList", searchMap));

        return searchMap;
    }
    
    /**
     * 직원개인기여도평가 평가등급배분표 관리 상세화면
     * @param      
     * @return String  
     * @throws 
     */
    public SearchMap evalConGradeModify(SearchMap searchMap) {
        String stMode = searchMap.getString("mode");
        
        if("MOD".equals(stMode)) {
            searchMap.addList("detail", getDetail("prs.evalCon.evalConGrade.getDetail", searchMap));
        }
    ...
    /**
     * 직원개인기여도평가 평가등급배분표 관리 등록/수정/삭제
     * @param      
     * @return String  
     * @throws 
     */
    public SearchMap evalConGradeProcess(SearchMap searchMap) {
    ...
    public SearchMap insertDB(SearchMap searchMap) {
        ...
        try {
            ...
            returnMap = insertData("prs.evalCon.evalConGrade.insertData", searchMap);
    ...
    public SearchMap updateDB(SearchMap searchMap) {
        ...
        try {
            ...
            returnMap = updateData("prs.evalCon.evalConGrade.updateData", searchMap);
        ...
        try {
           ...
           /* if(null !=  && 0 < .length) {
                for (int i = 0; i < .length; i++) {
                    ...
                    returnMap = updateData("prs.evalCon.evalConGrade.deleteData", searchMap);
                }
            }*/
    ...
    /**
     * 직원개인기여도평가 평가등급배분표 관리 엑셀다운로드
     * @param      
     * @return String  
     * @throws 
     */  
    public SearchMap evalConGradeListExcel(SearchMap searchMap) {
        String excelFileName = "직원개인기여도평가 평가등급배분표 관리";
        String excelTitle = "직원개인기여도평가 평가등급배분표 관리 리스트";
    	...
    	ArrayList iqueryItemList = (ArrayList)getList("prs.evalCon.evalConGrade.getCodeList", searchMap);
    	String[] itemArray = new String[0]; 
    	...
    	searchMap.put("excelDataList", (ArrayList)getList("prs.evalCon.evalConGrade.getEvalConGradeList", searchMap));
        	...
        	if(null != excelDataList && 0 < excelDataList.size()) {
                ...
                for(int i=0; i< strCode.length ; i++) {
                	...
                	if( 0 == i ){ // 평가년도  설정  부분
                		...
                		returnMap = insertData("prs.evalCon.evalConGrade.deleteEvalConGrade", searchMap);
	                	...
	                	for(int j = 0 ; j < colCnt ; j++ ){
		                	if(!"".equals(StaticUtil.nullToBlank(strCode[i])) && !"".equals(StaticUtil.nullToBlank(strValue[i][j]))) {
			                	...
			                	returnMap = insertData("prs.evalCon.evalConGrade.insertEvalConGrade", searchMap);
			                	inputValCnt++;
		                	}
	                	}
                	}
