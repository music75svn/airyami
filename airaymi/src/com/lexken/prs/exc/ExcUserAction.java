...
public class ExcUserAction extends CommonService {
     ...
     * @throws 
     */
    public SearchMap excUserList(SearchMap searchMap) {
     ...
     * @throws 
     */
    public SearchMap excUserList_xml(SearchMap searchMap) {
    	
        searchMap.addList("list", getList("prs.exc.excUser.getList", searchMap));
     ...
     * @throws 
     */
    public SearchMap excUserModify(SearchMap searchMap) {
    	String stMode = searchMap.getString("mode");
    	
    	HashMap detail = new HashMap();
    	
    	if("MOD".equals(stMode)) {
    		detail = getDetail("prs.exc.excUser.getDetail", searchMap);
    		searchMap.addList("detail", detail);
    		//관리조직 조회 
	        searchMap.addList("deptList", getList("prs.exc.excUser.getDeptList", searchMap));
	        searchMap.addList("posTcList", getList("prs.exc.excUser.getPosTcList", searchMap));
    	}
     ...
     * @throws 
     */
    public SearchMap excUserProcess(SearchMap searchMap) {
    ...
    public SearchMap insertDB(SearchMap searchMap) {
        ...
        try {
	         ...
	         * 간부 별도평가군 코드 채번 
	         **********************************/
	        String excGrpId = getStr("prs.exc.excUser.getExcGrpId", searchMap);
	        searchMap.put("excGrpId", excGrpId);
	        
	        /**********************************
	         * 간부 별도평가군 / 부서 / 직위 입력
	         **********************************/
        	returnMap = insertData("prs.exc.excUser.insertData", searchMap);
        	
	        String[] deptCds = searchMap.getStringArray("deptId");
	        String[] posTcs = searchMap.getStringArray("posTc");
        
        	returnMap = updateData("prs.exc.excUser.deleteDeptData", searchMap, true);
        	returnMap = updateData("prs.exc.excUser.deletePosData", searchMap, true);

		    for (int i = 0; i < deptCds.length; i++) {
		    	if(!deptCds[i].equals("")){
		        		searchMap.put("deptCd", deptCds[i]);
		        		returnMap = insertData("prs.exc.excUser.insertDeptData", searchMap);
		    	}
		    }		
		    for (int i = 0; i < posTcs.length; i++) {
		    	if(!posTcs[i].equals("")){
		        		searchMap.put("posTc", posTcs[i]);
		        		returnMap = insertData("prs.exc.excUser.insertPosData", searchMap);
		    	}
		    }
    ...
    public SearchMap insertAutoDB(SearchMap searchMap) {
     ...
     try {
	       ...
	       **********************************/
            ...
            returnMap = insertData("prs.exc.excUser.deleteUseDataAuto", searchMap);
		  	returnMap = insertData("prs.exc.excUser.insertUseDataAuto", searchMap);
		  	returnMap = insertData("prs.exc.excUser.insertLessThan1Months", searchMap);
    ...
    public SearchMap updateDB(SearchMap searchMap) {
        ...
        try {
	        	...
	        	returnMap = updateData("prs.exc.excUser.updateData", searchMap);
	    ...
	    try {
	        ...
	        if(null != excGrpIds && 0 < excGrpIds.length) {
		        for (int i = 0; i < excGrpIds.length; i++) {
		            ...
		            returnMap = updateData("prs.exc.excUser.deleteData", searchMap);
		            returnMap = updateData("prs.exc.excUser.deleteDeptData", searchMap, true);
		        	returnMap = updateData("prs.exc.excUser.deletePosData", searchMap, true);
		        }
		    }
     ...
     * @throws 
     */
    public SearchMap excUserUseList(SearchMap searchMap) {
    	
        searchMap.addList("excGrpUseDatail", getDetail("prs.exc.excUser.getExcGrpUseDatail", searchMap));
     ...
     * @throws 
     */
    public SearchMap excUserUseList_xml(SearchMap searchMap) {
        
        searchMap.addList("list", getList("prs.exc.excUser.getUseList", searchMap));
     ...
     * @throws 
     */
    public SearchMap excUserUseModify(SearchMap searchMap) {
    	
    	searchMap.addList("excGrpUseDatail", getDetail("prs.exc.excUser.getExcGrpUseDatail", searchMap));
    	searchMap.addList("insaUseList", getList("prs.exc.excUser.getInsaUseList", searchMap));    	
        searchMap.addList("userList", getList("prs.exc.excUser.getUseList", searchMap));
     ...
     * @throws 
     */
    public SearchMap excUserUseProcess(SearchMap searchMap) {
        HashMap returnMap = new HashMap();
        String stMode = searchMap.getString("mode");
    ...
    public SearchMap insertUseDB(SearchMap searchMap) {
        ...
        returnMap = updateData("prs.exc.excUser.deleteUseData", searchMap, true); 
        
        if(!"".equals(empn)) {
	        for (int i = 0; i < empns.length; i++) {
	        	searchMap.put("empn", empns[i]);
	        	returnMap = insertData("prs.exc.excUser.insertUseData", searchMap);
	        }
        }
	    ...
	    try {
	        ...
	        if(null != empns && 0 < empns.length) {
		        for (int i = 0; i < empns.length; i++) {
		            ...
		            searchMap.put("empn", empns[i]);
		            returnMap = updateData("prs.exc.excUser.deleteUseData", searchMap);
		        }
		    }
     ...
     * @throws 
     */  
    public SearchMap excUserListExcel(SearchMap searchMap) {
    	...
    	excelInfoList.add(new ExcelVO("조직업적평가", 	"BSC_RATE",		"right"));
    	excelInfoList.add(new ExcelVO("개인업적평가", 	"PRS_RATE",		"right"));
    	excelInfoList.add(new ExcelVO("종합경영효율성", 	"EFF_RATE",		"right"));
    	...
    	searchMap.put("excelDataList", (ArrayList)getList("prs.exc.excUser.getList", searchMap));
