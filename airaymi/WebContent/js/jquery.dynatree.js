...
DynaTreeNode.prototype = {
	...
	toDict: function(recursive, callback) {
		...
		dict.k = ( this.tree.activeNode === this );
...
//		this.tree.logDebug("dtnode.focus(): %o", this);
	...
	_activate: function(flag, fireEvents) {
		...
		if( flag ) {
			...
			//this.toggleExpand();  //조직명 클릭시 하위 조직보이도록 추가(2012.7.13 하윤식)
...
//		return "DynaTree '" + this.options.title + "'";
	...
	//2012.08.13 하윤식 추가
	serializeString: function(stopOnParents) {
		// Return a JavaScript array of objects, ready to be encoded as a JSON
		// string for selected nodes
		var nodeList = this.getSelectedNodes(stopOnParents),
			name = this.$tree.attr("name") || this.$tree.attr("id"),
			str = "";
		for(var i=0, l=nodeList.length; i<l; i++){
			str = str + nodeList[i].data.key + "|";
		}
		return str;
	},
	
	//2012.08.13 하윤식 추가
	serializeMetricString: function(stopOnParents) {
		// Return a JavaScript array of objects, ready to be encoded as a JSON
		// string for selected nodes
		var nodeList = this.getSelectedNodes(stopOnParents),
			name = this.$tree.attr("name") || this.$tree.attr("id"),
		...
		for(var i=0, l=nodeList.length; i<l; i++){
			if(nodeList[i].data.key.substring(0,1) == "M") { 
				if(nodeList[i].data.hideCheckbox != true)
					str = str + nodeList[i].data.key + "|";
			}
		}
		return str;
	...
	//2013.07.04 정용훈 추가
	serializeManKpiIdString: function(stopOnParents) {
		// Return a JavaScript array of objects, ready to be encoded as a JSON
		// string for selected nodes
		var nodeList = this.getSelectedNodes(stopOnParents),
		name = this.$tree.attr("name") || this.$tree.attr("id"),
		...
		for(var i=0, l=nodeList.length; i<l; i++){
			if(nodeList[i].data.key.substring(0,1) == "K") { 
				if(nodeList[i].data.hideCheckbox != true)
					str = str + nodeList[i].data.key + "|";
			}
		}
	...
	//2012.08.13 하윤식 추가
	serializeSubjectMetricString: function(stopOnParents) {
		// Return a JavaScript array of objects, ready to be encoded as a JSON
		// string for selected nodes
		var nodeList = this.getSelectedNodes(stopOnParents),
			name = this.$tree.attr("name") || this.$tree.attr("id"),
		...
		for(var i=0, l=nodeList.length; i<l; i++){
			if(nodeList[i].data.key.substring(0,1) == "S") { 
				if(nodeList[i].data.hideCheckbox != true)
					str = str + nodeList[i].data.key + "|";
			}
		}
