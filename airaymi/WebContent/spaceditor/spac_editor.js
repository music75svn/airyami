...
var base_dir = "/kgsbsc/spaceditor";
...
//			"emoticon2|이모티콘2 삽입|viewEmoticonWin()|4",
	...
	for(i = 0; i < toolbar_array.length; i++) {
		...
		group	= values[3];
		except_ok = 0;
		for(var j = 0; j < except_array.length; j++) {
...
function viewFullScreen(content_name) {
	...
	var full_height = (box_obj.clientHeight + 400 );
...
function viewImageUploadForm(event) {
	...
	var table = "";
		...
		table		+= "<div style='float:right;width:auto;padding:3px'><img src='"+base_dir+"/images/btn_close2.png' style='cursor:pointer' onClick='hideBlindScreen()' alt='close property window' title='닫기'></div>";
		table		+= "<div class='textboxes_contents'>";
		table		+= "<form name='uploadImageForm' method='post' enctype='multipart/form-data' action='"+base_dir+"/spac_editor_attach.jsp' target='spaceditor_upload_frame' onSubmit='return uploadImage(this)'>";
...
function viewFileUploadForm(event) {
	...
	var table = "";
		...
		table		+= "<div style='float:right;width:auto;padding:3px'><img src='"+base_dir+"/images/btn_close2.png' style='cursor:pointer' onClick='hideBlindScreen()' alt='close property window' title='닫기'></div>";
		table		+= "<div class='textboxes_contents'>";
		table		+= "<form name='uploadAttachForm' method='post' enctype='multipart/form-data' action='"+base_dir+"/spac_editor_attach.jsp' target='spaceditor_upload_frame' onSubmit='return uploadAttach(this)'>";
...
function removeFileLink(file_full_path) {
	if(confirm("파일을 삭제하시겠습니까?")) {
		spaceditor_filelistbox_frame.document.location.href=""+base_dir+"/spac_editor_attach.jsp?type=dlt&file="+file_full_path;
	} else {
		return;
	}
}
