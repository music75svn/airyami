...
public class IsoCertificationAction extends CommonService {
     ...
     * @throws
     */
    public SearchMap isoCertificationList(SearchMap searchMap) {

    	if ("".equals(StaticUtil.nullToBlank((String)searchMap.get("findDeptId")))) {
        	
			searchMap.put("findDeptId", getStr("bsc.popup.isoCertification.getScDeptMapping", searchMap));
		}

        searchMap.addList("deptInfo", getList("bsc.popup.isoCertification.getDeptInfo", searchMap));

    	searchMap.addList("itemInfo", getList("bsc.popup.isoCertification.getItem", searchMap));

        return searchMap;
    }

    /**
     * ISO 인증 매출액 데이터 조회(xml)
     * @param
     * @return String
     * @throws
     */
    public SearchMap isoCertificationList_xml(SearchMap searchMap) {

    	String itemCd = searchMap.getString("findItemCd");

    		searchMap.addList("list", getList("bsc.popup.isoCertification.getList", searchMap));
    ...
    public SearchMap excel(SearchMap searchMap) {
    	String excelFileName = "ISO 인증 매출액 실적";
    	String excelTitle = "ISO 인증 매출액 실적";
    	...
    	searchMap.put("excelDataList", (ArrayList)getList("bsc.popup.isoCertification.getList", searchMap));
    	return searchMap;
    }
