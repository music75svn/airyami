...
public class CountryExpProofAction extends CommonService {
     ...
     * @throws
     */
    public SearchMap countryExpProofList(SearchMap searchMap) {

    	if ("".equals(StaticUtil.nullToBlank((String)searchMap.get("findDeptId")))) {
        	
			searchMap.put("findDeptId", getStr("bsc.popup.countryExpProof.getScDeptMapping", searchMap));
		}

        searchMap.addList("deptInfo", getList("bsc.popup.countryExpProof.getDeptInfo", searchMap));

    	searchMap.addList("itemInfo", getList("bsc.popup.countryExpProof.getItem", searchMap));

        return searchMap;
    }

    /**
     * 국내 방폭 인증 매출액(억원) 데이터 조회(xml)
     * @param
     * @return String
     * @throws
     */
    public SearchMap countryExpProofList_xml(SearchMap searchMap) {

    	String itemCd = searchMap.getString("findItemCd");

    		searchMap.addList("list", getList("bsc.popup.countryExpProof.getList", searchMap));
    ...
    public SearchMap excel(SearchMap searchMap) {
    	String excelFileName = "국내 방폭 인증 매출액(억원) 실적";
    	String excelTitle = "국내 방폭 인증 매출액(억원) 실적";
    	...
    	searchMap.put("excelDataList", (ArrayList)getList("bsc.popup.countryExpProof.getList", searchMap));
    	return searchMap;
    }
