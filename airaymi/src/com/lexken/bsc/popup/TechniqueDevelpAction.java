/*************************************************************************
* CLASS 명      : techniqueDevelp
* 작 업 자      : 안요한
* 작 업 일      : 2013년 08월 13일
* 기    능      : 가스발전관련 기업 기술지원 수입실적
* ---------------------------- 변 경 이 력 --------------------------------
...
public class TechniqueDevelpAction extends CommonService {

    private static final long serialVersionUID = 1L;

    /**
     * 가스발전관련 기업 기술지원 수입실적 조회
     * @param
     * @return String
     * @throws
     */
    public SearchMap techniqueDevelpList(SearchMap searchMap) {

    	if ("".equals(StaticUtil.nullToBlank((String)searchMap.get("findDeptId")))) {
        	
			searchMap.put("findDeptId", getStr("bsc.popup.techniqueDevelp.getScDeptMapping", searchMap));
		}

        searchMap.addList("deptInfo", getList("bsc.popup.techniqueDevelp.getDeptInfo", searchMap));

    	searchMap.addList("itemInfo", getList("bsc.popup.techniqueDevelp.getItem", searchMap));

        return searchMap;
    }

    /**
     * 가스발전관련 기업 기술지원 수입실적 데이터 조회(xml)
     * @param
     * @return String
     * @throws
     */
    public SearchMap techniqueDevelpList_xml(SearchMap searchMap) {

    	String itemCd = searchMap.getString("findItemCd");

    		searchMap.addList("list", getList("bsc.popup.techniqueDevelp.getList", searchMap));
    ...
    public SearchMap excel(SearchMap searchMap) {
    	String excelFileName = "가스발전관련 기업 기술지원 수입실적 실적";
    	String excelTitle = "가스발전관련 기업 기술지원 수입실적 실적";
    	...
    	searchMap.put("excelDataList", (ArrayList)getList("bsc.popup.techniqueDevelp.getList", searchMap));
    	return searchMap;
    }
