/*************************************************************************
* CLASS 명      : checkServiceTimelyRateUse
* 작 업 자      : 김기현
* 작 업 일      : 2012년 6월 18일
* 기    능      : 시스템항목관리
* ---------------------------- 변 경 이 력 --------------------------------
...
public class CheckServiceTimelyRateUseAction extends CommonService {
     ...
     * @throws
     */
    public SearchMap checkServiceTimelyRateUseList(SearchMap searchMap) {

    	if ("".equals(StaticUtil.nullToBlank((String)searchMap.get("findDeptId")))) {
    		searchMap.put("findDeptId", getStr("bsc.popup.checkServiceTimelyRateUse.getScDeptMapping", searchMap));
    	}

    	searchMap.addList("deptList", getList("bsc.popup.checkServiceTimelyRateUse.getDeptList", searchMap));

    	searchMap.addList("itemInfo", getList("bsc.popup.checkServiceTimelyRateUse.getItem", searchMap));
     ...
     * @throws
     */
    public SearchMap checkServiceTimelyRateUseList_xml(SearchMap searchMap) {

        searchMap.addList("list", getList("bsc.popup.checkServiceTimelyRateUse.getList", searchMap));
     ...
     * @throws
     */
    public SearchMap checkServiceTimelyRateUseProcess(SearchMap searchMap) {
    ...
    public SearchMap excel(SearchMap searchMap) {
    	...
    	String procYn = searchMap.getString("PROC_YN");
 ...
 //   	excelInfoList.add(new ExcelVO("부서명", "DEPT_NM", "left"));
    	...
    	excelInfoList.add(new ExcelVO("접수기간", "RCPT_DATE", "left"));
    	...
    	ArrayList list = (ArrayList)getList("bsc.popup.checkServiceTimelyRateUse.getList", searchMap);
    	
    	for (int i = 0; i < list.size(); i++) {
    		if (((HashMap)list.get(i)).get("PROC_YN").equals("T")) {
    			((HashMap)list.get(i)).put("PROC_YN_NM", "적시");
    		} else {
    			((HashMap)list.get(i)).put("PROC_YN_NM", "미적시");
    		}
    	}
