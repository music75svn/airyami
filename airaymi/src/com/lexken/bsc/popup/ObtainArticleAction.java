...
public class ObtainArticleAction extends CommonService {
     ...
     * @throws
     */
    public SearchMap obtainArticleList(SearchMap searchMap) {

    	if ("".equals(StaticUtil.nullToBlank((String)searchMap.get("findDeptId")))) {
        	
			searchMap.put("findDeptId", getStr("bsc.popup.obtainArticle.getScDeptMapping", searchMap));
		}

        searchMap.addList("deptInfo", getList("bsc.popup.obtainArticle.getDeptInfo", searchMap));

    	searchMap.addList("itemInfo", getList("bsc.popup.obtainArticle.getItem", searchMap));

        return searchMap;
    }

    /**
     * 조달물품대행 검사수입 데이터 조회(xml)
     * @param
     * @return String
     * @throws
     */
    public SearchMap obtainArticleList_xml(SearchMap searchMap) {

    	String itemCd = searchMap.getString("findItemCd");

    		searchMap.addList("list", getList("bsc.popup.obtainArticle.getList", searchMap));
    ...
    public SearchMap excel(SearchMap searchMap) {
    	String excelFileName = "조달물품대행 검사수입 실적";
    	String excelTitle = "조달물품대행 검사수입 실적";
    	...
    	excelInfoList.add(new ExcelVO("사업 분류", "RPT_TC_NM", "center"));
    	excelInfoList.add(new ExcelVO("업무 구분", "ROLE_TC_NM", "center"));
    	excelInfoList.add(new ExcelVO("검사 종류", "INSP_KIND_TC_NM", "center"));
    	...
    	searchMap.put("excelDataList", (ArrayList)getList("bsc.popup.obtainArticle.getList", searchMap));
    	return searchMap;
    }
