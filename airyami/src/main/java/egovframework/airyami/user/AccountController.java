...
public class AccountController {
     ...
     * 회사 계좌관리 팝업 이동 
     */
    @RequestMapping(value="/account/accountListPop.do")
    public String accountFindListPop(HttpServletRequest request, HttpServletResponse response, 
    	...
    	// 화폐코드 조회
    	params.put( "CODE_GROUP_ID", "CURRENCY" ); //화폐코드
	 ...
	 * @exception Exception
	 */
    @RequestMapping(value="/account/selectAccountList.do")
    public String selectAccountList(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
    	...
    	*/
    	...
    	try{
    		if("N".equals(excelYn)){
    			...
    			int totCnt = cmmService.getCommDbInt(params, "account.getAccountCount");
    		...
    		List<ValueMap> list = cmmService.getCommDbList(params, "account.getAccountList");
     ...
     * 회사 계좌관리 detail 상세조회 
     */
    @RequestMapping(value="/account/getAccountDetail.do")
    public String getAccountDetail(HttpServletRequest request, HttpServletResponse response, 
    		ModelMap model) throws Exception {
    	Map<String,Object> params = CommonUtils.getRequestMap(request);
    	log.debug("param :: " + params);
    	...
    	try{
    		ValueMap ds_detail = cmmService.getCommDbMap(params, "account.getAccountDetail");
     ...
     * 회사 계좌관리 detail 저장 
     */
    @RequestMapping(value="/account/saveAccount.do")
    public String saveAccount(HttpServletRequest request, HttpServletResponse response, 
    	...
    	try{
    		...
    		if("CREATE".equals(params.get("PROC_MODE"))){
    			...
    			cmmService.insertCommDb(params, "account.insertAccount");
    		}else if("UPDATE".equals(params.get("PROC_MODE"))){
    			cmmService.updateCommDb(params, "account.updateAccount");
    		}else if("DELETE".equals(params.get("PROC_MODE"))){
    			cmmService.deleteCommDb(params, "account.deleteAccount");
    		}
    	}
    	catch(Exception e){
    		success = false;
