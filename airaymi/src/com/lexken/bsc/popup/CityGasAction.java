...
public class CityGasAction extends CommonService {

    private static final long serialVersionUID = 1L;

    /**
     * 도시가스품질검사 목표대비 처리 건수 조회
     * @param
     * @return String
     * @throws
     */
    public SearchMap cityGasList(SearchMap searchMap) {

    	if ("".equals(StaticUtil.nullToBlank((String)searchMap.get("findDeptId")))) {
        	
			searchMap.put("findDeptId", getStr("bsc.popup.cityGas.getScDeptMapping", searchMap));
		}

        searchMap.addList("deptInfo", getList("bsc.popup.cityGas.getDeptInfo", searchMap));

    	searchMap.addList("itemInfo", getList("bsc.popup.cityGas.getItem", searchMap));

        return searchMap;
    }

    /**
     * 도시가스품질검사 목표대비 처리 건수 데이터 조회(xml)
     * @param
     * @return String
     * @throws
     */
    public SearchMap cityGasList_xml(SearchMap searchMap) {

    	String itemCd = searchMap.getString("findItemCd");

    		searchMap.addList("list", getList("bsc.popup.cityGas.getList", searchMap));
    ...
    public SearchMap excel(SearchMap searchMap) {
    	String excelFileName = "도시가스품질검사 목표대비 처리 건수 실적";
    	String excelTitle = "도시가스품질검사 목표대비 처리 건수 실적";
    	...
    	searchMap.put("excelDataList", (ArrayList)getList("bsc.popup.cityGas.getList", searchMap));
    	return searchMap;
    }
