...
?>
<?php 
require_once("spac_editor.class.php"); 
		...
		<td width="200" valign="top">
			<div class="tree">
				<div>
					...
					<?php if(!$SpacEditor->get_dir) : ?>☜<?php endif; ?>
				</div>			
			...
			<div style="margin-top:3px;">
				<form name='make_folder_form' method="POST" action="spac_editor_file_tree.php" onSubmit="return checkMakeFolderForm(this)">
					<input type="hidden" name="get_dir" value="<?=$SpacEditor->get_dir?>">
							...
							?>
								<div id="file_unit_box_<?=$loop_count?>" class="<?=$box_class?>" onMouseOver="fileboxOver(this)"  onMouseOut="fileboxOut(this,'<?=$loop_count?>')">
									<div class="image_info_title">
										<input type="checkbox" name="chk_file[]" id="chk_<?=$loop_count?>" value="<?=$file['name']?>" title="<?=$file['name']?>" <?=$checked?> onClick="checkFile2(this,'<?=$loop_count?>')"><label for="chk_<?=$loop_count?>"  title="<?=$file['name']?>"><?=$SpacEditor->resizeString($file['name'],100)?></label>
									</div>
									<div class="image_info_image">
										<img src="<?=$file['thumb_url']?>"  alt="<?=$file['thumb_name']?>" width="<?=$file['thumb_width']?>" height="<?=$file['thumb_height']?>" onClick="checkFile('<?=$loop_count?>')" ondblclick="insertImageOpener('<?=$file['name']?>','<?=$file['file_url']?>','<?=$file['width']?>','<?=$file['height']?>')" title="한번클릭시 선택, 더블클릭시 에디터에 입력됩니다."  onmouseup="viewFileContextMenu('<?=$SpacEditor->get_dir?>','<?=$file['name']?>','<?=$file['width']?>','<?=$file['height']?>','<?=$file['thumb_url']?>','<?=$file['file_url']?>',event)">
									</div>
